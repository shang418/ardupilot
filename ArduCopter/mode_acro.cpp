...
#define ACRO_SPEED_0                         10      // speed below which it is always safe to switch to loiter

// 400hz loop update rate
#define ACRO_BRAKE_TIME_ESTIMATE_MAX         (600*4) // max number of cycles the brake will be applied before we switch to loiter
#define ACRO_BRAKE_TO_LOITER_TIMER           (150*4) // Number of cycles to transition from brake mode to loiter mode.  Must be lower than POSHOLD_LOITER_STAB_TIMER
#define ACRO_WIND_COMP_START_TIMER           (150*4) // Number of cycles to start wind compensation update after loiter is engaged
#define ACRO_CONTROLLER_TO_PILOT_MIX_TIMER   (50*4)  // Set it from 100 to 200, the number of centiseconds loiter and manual commands are mixed to make a smooth transition.
#define ACRO_SMOOTH_RATE_FACTOR              0.0125f // filter applied to pilot's roll/pitch input as it returns to center.  A lower number will cause the roll/pitch to return to zero more slowly if the brake_rate is also low.
#define ACRO_WIND_COMP_TIMER_10HZ            40      // counter value used to reduce wind compensation to 10hz
...
#define ACRO_STICK_RELEASE_SMOOTH_ANGLE      1800    // max angle required (in centi-degrees) after which the smooth stick release effect is applied
#define ACRO_WIND_COMP_ESTIMATE_SPEED_MAX    10      // wind compensation estimates will only run when velocity is at or below this speed in cm/s
#define ACRO_WIND_COMP_LEAN_PCT_MAX          0.6666f // wind compensation no more than 2/3rds of angle max to ensure pilot can always override
 ...
 void ModeAcro::run()
{
    ...
    switch (roll_mode) {
        ...
        case RPMode::BRAKE_READY_TO_LOITER:
            ...
            if ((fabsf(vel_right) <= ACRO_SPEED_0) && (brake.timeout_roll > 50*LOOP_RATE_FACTOR)) {
    ...
    switch (pitch_mode) {
        ...
        case RPMode::PILOT_OVERRIDE:
            ...
            if (is_zero(target_pitch) && (fabsf(pilot_pitch) < 2 * g.poshold_brake_rate)) {
                ...
                brake.timeout_pitch = ACRO_BRAKE_TIME_ESTIMATE_MAX; // number of cycles the brake will be applied, updated during braking mode.
        ...
        case RPMode::CONTROLLER_TO_PILOT_OVERRIDE:
            ...
            // calculate controller_to_pilot mix ratio
            controller_to_pilot_pitch_mix = (float)controller_to_pilot_timer_pitch / (float)ACRO_CONTROLLER_TO_PILOT_MIX_TIMER;
...
void ModeAcro::update_pilot_lean_angle(float &lean_angle_filtered, float &lean_angle_raw)
{
    // if raw input is large or reversing the vehicle's lean angle immediately set the filtered angle to the new raw angle
    if ((lean_angle_filtered > 0 && lean_angle_raw < 0) || (lean_angle_filtered < 0 && lean_angle_raw > 0) || (fabsf(lean_angle_raw) > ACRO_STICK_RELEASE_SMOOTH_ANGLE)) {
        ...
        if (lean_angle_filtered > 0) {
            ...
            lean_angle_filtered -= MAX(lean_angle_filtered * ACRO_SMOOTH_RATE_FACTOR, MAX(1.0f, g.poshold_brake_rate/(float)LOOP_RATE_FACTOR));
...
float ModeAcro::mix_controls(float mix_ratio, float first_control, float second_control)
{
    mix_ratio = constrain_float(mix_ratio, 0.0f, 1.0f);
    return mix_ratio * first_control + (1.0f - mix_ratio) * second_control;
}

void ModeAcro::update_brake_angle_from_velocity(float &brake_angle, float velocity)
{
    float lean_angle;
...
void ModeAcro::init_wind_comp_estimate()
{
    wind_comp_ef.zero();
    wind_comp_timer = 0;
    wind_comp_roll = 0.0f;
...
void ModeAcro::update_wind_comp_estimate()
{
    ...
    if (inertial_nav.get_speed_xy_cms() > ACRO_WIND_COMP_ESTIMATE_SPEED_MAX) {
        return;
    }
    ...
    // limit acceleration
    const float accel_lim_cmss = tanf(radians(ACRO_WIND_COMP_LEAN_PCT_MAX * copter.aparm.angle_max * 0.01f)) * (GRAVITY_MSS*100);
...
void ModeAcro::get_wind_comp_lean_angles(float &roll_angle, float &pitch_angle)
{
    // reduce rate to 10hz
    wind_comp_timer++;
    if (wind_comp_timer < ACRO_WIND_COMP_TIMER_10HZ) {
        return;
    }
    wind_comp_timer = 0;
...
void ModeAcro::roll_controller_to_pilot_override()
{
    roll_mode = RPMode::CONTROLLER_TO_PILOT_OVERRIDE;
    controller_to_pilot_timer_roll = ACRO_CONTROLLER_TO_PILOT_MIX_TIMER;
...
void ModeAcro::pitch_controller_to_pilot_override()
{
    pitch_mode = RPMode::CONTROLLER_TO_PILOT_OVERRIDE;
    controller_to_pilot_timer_pitch = ACRO_CONTROLLER_TO_PILOT_MIX_TIMER;
