...
try
    state = load('bprl_copter','copter');
catch
    run('bprl_copter.m')
    fprintf('Could not find bprl_copter.mat file, running copter.m\n')
    return
end
...
for i = 1:numel(state.copter.motors)
    ...
    [thrust,torque] = fit_pwm_to_thrust_torque(pwm_in(motor.channel));
    
    %sprintf("Thrust: %0.4f, Torque:%0.4f",thrust,torque)
    ...
    moment_yaw = -torque * motor.direction;
     ...
     sprintf("Motor i:%d Roll Moment: %0.4f, Pitch Moment:%0.4f",i,moment_roll,moment_pitch)
...
moments = [-sum([state.copter.motors.moment_roll]);sum([state.copter.motors.moment_pitch]);sum([state.copter.motors.moment_yaw])] - 0*rotational_drag;
