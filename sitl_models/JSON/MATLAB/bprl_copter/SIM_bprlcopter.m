...
for i = 1:numel(state.copter.motors)
    ...
    state.copter.motors(i).thrust_pwm_prev= 0; 
    state.copter.motors(i).torque = 0;
    state.copter.motors(i).torque_pwm_prev = 0; 
end
...
p_coeffs_torque = [-0.0003 -0.0060 0.0118 0.0984 0.1041];
mean_fit = 1546;
std_fit = 255.6;
...
for i = 1:numel(state.copter.motors)
    ...
    % alpha = exp(-2*state.delta_t);
    % 
    % thrust = alpha*motor.thrust + (1-alpha)*motor.thrust_pwm_prev;
    % torque = alpha*motor.torque + (1-alpha)*motor.torque_pwm_prev;
